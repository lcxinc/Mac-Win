...
DOCUMENTATION_CANONICAL = f"""# Mac-Win migration asset inventory

## Evidence identity and scope

The immutable source is commit
`{SOURCE_COMMIT}`, identified by annotated tag
...
class AssetDocumentationTests(unittest.TestCase):
    ...
    def assertVerificationBoundaries(self, text):
        normalized = " ".join(text.split())
        required = (
            "`migration/assets/metadata-policy.json` is a bounded, closed, manually reviewed input, not an eighth generated output.",
            "No canonical serialization claim is made for the manual policy.",
            "Canonical LF serialization and exact-output comparison apply only to the seven generated JSON documents.",
            "`--check` compares the seven current worktree output files with in-memory expected bytes; it does not bind the stage-0 index.",
            "`validate_migration_asset_inventory.py` binds the manual policy's current worktree bytes to its stage-0 index blob and validates the closed policy.",
            "It then requires the worktree bytes and stage-0 index blob for each of the seven generated documents to equal the in-memory expected bytes",
        )
        forbidden = (
            "Every policy or generated JSON document",
            "`--check` regenerates in memory and compares exact committed bytes.",
        )
    ...
    def test_rejects_the_previous_overbroad_policy_and_check_claims(self):
        old_policy_claim = DOCUMENTATION_CANONICAL.replace(
            "`migration/assets/metadata-policy.json` is a bounded, closed, manually reviewed\n"
            "input, not an eighth generated output. It is limited to 64 KiB and parsed as\n"
            "strict UTF-8 JSON with duplicate-key rejection and a closed schema. No canonical\n"
            "serialization claim is made for the manual policy.\n\n"
            ...
            "serialized in the approved schema order, encoded as ASCII-compatible UTF-8\n"
            "JSON, and terminated with one LF newline.",
            "`migration/assets/metadata-policy.json` is the reviewed input policy, not an "
            "eighth generated output. Every policy or generated JSON document is bounded "
            "to 64 KiB, uses strict UTF-8 and LF, has a closed schema, and is serialized "
            "canonically.",
            1,
        )
        old_check_claim = DOCUMENTATION_CANONICAL.replace(
            "`--check` compares the seven current worktree output files with\n"
            "in-memory expected bytes; it does not bind the stage-0 index.",
            "`--check` regenerates in memory and compares exact committed bytes.",
            1,
        )
        for name, mutant in (
            ("policy canonicalization", old_policy_claim),
            ("check committed bytes", old_check_claim),
        ):
            with self.subTest(name=name):
                self.assertNotEqual(mutant, DOCUMENTATION_CANONICAL)
                with self.assertRaises(AssertionError):
                    self.assertVerificationBoundaries(mutant)
